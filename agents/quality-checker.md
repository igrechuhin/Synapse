...
- Use **only** Cortex MCP tool `run_quality_gate()` (zero-arg) or, as fallback, `.venv/bin/python .cortex/synapse/scripts/{language}/check_linting.py`, `check_file_sizes.py`, `check_function_lengths.py`, `check_test_naming.py` as needed. Do **NOT** run raw linter/check commands in a Shell.
