...
- Use **only** Cortex MCP tool `run_quality_gate()` or, as fallback, `.venv/bin/python .cortex/synapse/scripts/{language}/check_types.py`. Do **NOT** run raw type checker commands (e.g., `pyright`, `tsc`) in a Shell.
