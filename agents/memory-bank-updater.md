...
- **Roadmap edit discipline (CRITICAL)**: **ALL edits to roadmap.md** (including adding plan links, updating references, removing entries, or any other modifications) **MUST** be performed via `manage_file(operation='write', ...)` after reading current content with `manage_file(operation='read')`, or via the dedicated tools (`update_memory_bank(operation='roadmap_add'|'roadmap_remove'|'roadmap_remove_section')`, `plan(operation='register', ...)`). Do **NOT** use Write, StrReplace, or ApplyPatch on roadmap.md or any other memory-bank paths. This applies to all roadmap operations, including roadmap sync cleanup, link/archive tasks, and plan registration.
...
- **Adding a single new plan entry** (e.g. when invoked from Plan prompt): **MUST** use **`plan(operation="register", plan_title=..., description=..., section=...)`** or **`update_memory_bank(operation="roadmap_add", section=..., entry_text=...)`**. Do **not** build full roadmap content or call `manage_file(roadmap.md, write, content=...)` for single-entry adds—that causes corruption.
...
- **Removing a completed roadmap entry** (e.g. when invoked from implement Step 5): **MUST** use **`update_memory_bank(operation="roadmap_remove", entry_contains="<unique substring of the bullet>")`**. Do **not** read roadmap, build updated content, and call `manage_file(roadmap.md, write, content=...)`—that causes corruption.
- **Removing an entire roadmap section (heading + its list)**: After removing all bullets with `update_memory_bank(operation="roadmap_remove", ...)`, use **`update_memory_bank(operation="roadmap_remove_section", section_heading_contains="<heading substring>")`** to remove the orphan section (heading and content) without full-content write. Do **not** use `manage_file(roadmap.md, write, content=...)` for section removal—that causes corruption. If the tool is unavailable, prefer leaving the empty heading in place; only as last resort use a single minimal write that deletes just that block.
...
- **Adding one progress entry**: **MUST** use **`update_memory_bank(operation="progress_append", date_str="YYYY-MM-DD", entry_text="**Title** - COMPLETE. Summary...")`**. Do **not** build full progress content or call `manage_file(progress.md, write, content=...)`.
- **Adding one completed work entry to activeContext**: **MUST** use **`update_memory_bank(operation="active_context_append", date_str="YYYY-MM-DD", title="...", summary="...")`**. Do **not** build full activeContext content or call `manage_file(activeContext.md, write, content=...)` for this update.
