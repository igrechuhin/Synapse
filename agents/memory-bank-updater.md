...
- **Removing an entire roadmap section (heading + its list)**: Prefer leaving the now-empty heading in place after removing all bullets with `remove_roadmap_entry`. Only if the heading itself must be removed, perform a single, minimal `manage_file(roadmap.md, write, ...)` edit that deletes just that contiguous block (heading, optional intro paragraph, and its list) while preserving all other content verbatim. This pattern comes from the **Session Optimization: Roadmap Section Removal and Roadmap Sync Clarity** plan (`.cortex/plans/session-optimization-roadmap-section-removal-and-sync.md`) and avoids the full-file rewrite corruption that previously broke dates and phase names.
