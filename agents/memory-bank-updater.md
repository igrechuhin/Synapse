...
- **Removing an entire roadmap section (heading + its list)**: After removing all bullets with `roadmap(operation="remove_entry", ...)`, use **`roadmap(operation="remove_section", section_heading_contains="<heading substring>")`** to remove the orphan section (heading and content) without full-content write. Do **not** use `manage_file(roadmap.md, write, content=...)` for section removal—that causes corruption. If roadmap tool is unavailable, prefer leaving the empty heading in place; only as last resort use a single minimal write that deletes just that block.
