...
- **Adding a single new plan entry** (e.g. when invoked from create-plan): **MUST** use **`plan(operation="register", plan_title=..., description=..., section=...)`** or **`update_memory_bank(operation="roadmap_add", section=..., entry_text=...)`**. Do **not** build full roadmap content or call `manage_file(roadmap.md, write, content=...)` for single-entry adds—that causes corruption.
