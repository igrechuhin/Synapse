...
- **Roadmap edit discipline (CRITICAL)**: **ALL edits to roadmap.md** (including adding plan links, updating references, removing entries, or any other modifications) **MUST** be performed via `manage_file(operation='write', ...)` after reading current content with `manage_file(operation='read')`, or via the dedicated roadmap tools (`add_roadmap_entry`, `remove_roadmap_entry`, `register_plan_in_roadmap`, `remove_roadmap_section`). Do **NOT** use Write, StrReplace, or ApplyPatch on roadmap.md or any other memory-bank paths. This applies to all roadmap operations, including roadmap sync cleanup, link/archive tasks, and plan registration.
