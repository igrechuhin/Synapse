#!/usr/bin/env python3
"""Pre-commit hook to enforce file size limits for Go sources."""
...
def main() -> None:
    ...
    if files_from_env is not None:
        go_files = [f for f in files_from_env if f.suffix == ".go"]
    else:
        if not _allow_full_scan():
            print("✅ No FILES provided and ALLOW_FULL_SCAN!=1 (skipped)")
            sys.exit(0)

        go_files = sorted(project_root.rglob("*.go"))
        if not go_files:
            print("✅ No Go sources detected for full scan (skipped)")
            sys.exit(0)

    for go_file in go_files:
        lines = _count_logical_lines(go_file)
        if lines > MAX_LINES:
            violations.append((go_file, lines))
        elif lines > WARN_LINES:
            warnings_list.append((go_file, lines))
