...
- ⚠️ **CI parity gap — function length filtering**: The local `run_quality_gate()` applies a diff-aware filter: it suppresses function-length violations whose start line falls outside the changed diff hunks, or that are pre-existing (same or worse in HEAD). CI's `check_function_lengths.py` has **no such filter** — it checks every function in every file. If Phase A passes locally but any staged Python file contains functions longer than 30 logical lines (new **or** pre-existing), CI will fail. **Before writing the Phase A pass result**, scan every staged `.py` file: for each function, count its logical lines (blank lines, comments, and docstrings excluded). If any function exceeds 30 lines, fix it now — extract helpers, split into modules — before recording Phase A as passed.
